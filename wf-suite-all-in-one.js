...
// @grant        unsafeWindow
// ==/UserScript==
// Polyfill cho GM_getValue / GM_setValue khi chạy ngoài Tampermonkey
...
(function () {
  ...
  // ✅ BẢN MỚI: luôn đóng preview & chờ đóng xong trước khi qua asset tiếp theo
async function handleDimensionAction(img, valueTrue, fromBulk = false) {
  let performed = false;

  try {
    // Mở preview cho asset hiện tại
    img.click();
    await waitForElement(
    ...
    // Đọc giá trị HAS_DIMENSIONS hiện tại trong preview
    const currentDim = readHasDimensionsFromPreview(previewRoot);

    // CASE 1: Asset đã đúng TRUE/FALSE -> skip nhưng PHẢI đóng preview rồi mới qua asset tiếp theo
    if (currentDim === valueTrue) {
      console.log('WF Dim Helper: HAS_DIMENSIONS đã đúng, skip asset này.');

      // Gửi phím Esc để đóng preview
      document.dispatchEvent(new KeyboardEvent('keydown', {
        key: 'Escape',
        code: 'Escape',
        keyCode: 27,
      ...
      // Chờ preview đóng hẳn (max ~1.5s)
      const startSkip = Date.now();
      while (isPreviewOpen() && Date.now() - startSkip < 1500) {
        await sleep(100);
      }

      // Không làm gì thêm cho asset này
      return false;
    }

    // CASE 2: Cần đổi TRUE/FALSE -> set dropdown + Save Tags
    ...
    // Đảm bảo preview được đóng trước khi trả về (phòng khi nút Close không có)
    if (isPreviewOpen()) {
      document.dispatchEvent(new KeyboardEvent('keydown', {
        key: 'Escape',
        code: 'Escape',
